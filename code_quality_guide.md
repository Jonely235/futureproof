...
class DatabaseService {
  ...
  Future<String> addTransaction(Transaction t) async {
    ...
    try {
      ...
      _log.info('Transaction added: ${t.id}');
      _log.fine('Transaction details: amount=${t.amount}, category=${t.category}');
      return t.id;

    } catch (e, stackTrace) {
      _log.severe('Failed to add transaction ${t.id}', e, stackTrace);
      rethrow;
    }
  }

  Future<List<Transaction>> getAllTransactions() async {
    _log.fine('Loading all transactions');

    try {
      final db = await database;
      final maps = await db.query('transactions', orderBy: 'date DESC');

      _log.info('Loaded ${maps.length} transactions');

      if (maps.isEmpty) {
        _log.fine('No transactions found');
        return [];
      }

      final transactions = maps.map((map) => _transactionFromMap(map)).toList();

      _log.fine('Transaction dates: ${transactions.map((t) => t.date.toIso8601String()).take(3).join(', ')}...');

      return transactions;

    } catch (e, stackTrace) {
      _log.severe('Failed to load transactions', e, stackTrace);
...
```dart
// Log with context
_log.info('Transaction added', Transaction: t.toJson());

// Or use key-value pairs
_log.info('User action', action: 'add_transaction', id: t.id, amount: t.amount);
```
...
// 4. Use AutomaticKeepAliveClientMixin for state preservation
...
**Iteration**: 6 of 20
**Related Files**: tasks.md, STATUS.md
